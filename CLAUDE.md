...
- [x] **Phase 13 (cont.) — hardening pass**
  ...
  - End-to-end smoke test across the full stack via `docker compose up` — `scripts/smoke_e2e.sh`, run nightly in `.github/workflows/nightly.yml` (it builds every image, so it stays off the push path); the backend container now runs `alembic upgrade head` on startup, so a fresh `docker compose up` is fully provisioned. Nightly also runs a viewport regression audit (`frontend/scripts/audit-viewports.mjs`) that seeds the demo dataset and asserts every page fits at 375px and 1440px in headless Chrome (failure screenshots uploaded as an artifact)
