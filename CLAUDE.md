...
- [ ] **Phase 7 — NAS devices**
  - Resolve the `nas_devices` ↔ FreeRADIUS `nas` table sync strategy (open item in Prioritized Recommendations) before writing this phase's code
  - NAS devices API (RBAC-gated: `nas_devices:read/write`), shared secrets encrypted at rest
  - Full unit + integration coverage (validation, secret handling, CRUD, uniqueness constraints)
