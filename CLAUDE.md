...
- [x] **Over-quota enforcement job** — APScheduler interval job (`app/jobs/quota_enforcement.py`, default every 5 min, `quota_enforcement_interval_minutes`) polling the usage report and disconnecting live sessions of subscribers at/over quota via the existing pyrad CoA path (`disconnect_service`, `actor_id=None` for system actions). Opt-in per plan (`plans.enforce_quota`, added via migration `b2c3d4e5f6a7`; plan form toggle on the frontend); a per-subscriber cooldown (`quota_enforce_cooldown_minutes`, default 30) skips breaches already acted on; each enforced subscriber gets one `quota_enforced` audit entry (resource `subscribers`, admin_id NULL) summarizing per-session outcomes, alongside the transport-level `disconnect` entries.
- [x] **Usage overage billing** — monthly surcharge job (2nd of month at 00:15 UTC, after base generation + overdue sweep) billing per-GB excess beyond quota from the radacct usage report (`generate_overage_invoices` in `app/services/billing.py`, manual trigger `POST /api/v1/invoices/overage/generate`, `invoices:write`). Opt-in per plan via `plans.overage_price_per_gb` (migration `c3d4e5f6a7b8`; plan form field); fractional GB × rate rounded half-up to cents; idempotent per (subscriber, period) via a new `invoices.kind` column (`base` | `overage`, badged as `surcharge` in the invoices list/detail); the base-generation overlap check now scopes to `kind='base'` so the two never block each other.
