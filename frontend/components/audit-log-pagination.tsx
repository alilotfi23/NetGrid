...
type Props = {
  ...
  pageSize: number;
  total: number;
  /** Preserve the log filters when paging. */
  adminId?: string;
  action?: string;
  resource?: string;
};
...
/**
 * Pagination controls for the audit log table, backed by the API's page and
 * page_size params. Server-rendered links navigate pages; the page-size
 * select (a client control) resets to page 1. Both preserve the filters.
 */
export function AuditLogPagination({ page, pageSize, total, adminId, action, resource }: Props) {
        ...
        <nav aria-label="Audit log pages" className="flex items-center gap-1.5">
          ...
          {pageNumbers(current, totalPages).map((item, index) =>
            item === "…" ? (
              ...
              <Link key={item} href={buildHref(item, pageSize)} className={linkClass}>
                {item}
              </Link>
            ),
          )}
